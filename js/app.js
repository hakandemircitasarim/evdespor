// App shell: hash router + bottom navigation.
...
const NAV = [
  { id: 'today', label: 'TODAY', icon: '<svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="17" rx="2"/><path d="M3 9h18M8 2v4M16 2v4"/></svg>' },
  { id: 'program', label: 'PROGRAM', icon: '<svg viewBox="0 0 24 24"><path d="M4 6h16M4 12h16M4 18h10"/></svg>' },
  { id: 'progress', label: 'PROGRESS', icon: '<svg viewBox="0 0 24 24"><path d="M3 20h18M5 16l4-5 4 3 6-8"/></svg>' },
  { id: 'settings', label: 'SETTINGS', icon: '<svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3M4.9 4.9l2.1 2.1M17 17l2.1 2.1M4.9 19.1L7 17M17 7l2.1-2.1"/></svg>' },
];
...
export function render() {
  ...
  if (!hideNav) {
    const nav = el(`<nav class="bottom">${NAV.map(n => `<button data-nav="${n.id}" class="${n.id === name || (name === 'library' && n.id === 'program') ? 'active' : ''}">${n.icon}<span>${n.label}</span></button>`).join('')}</nav>`);
